...
\title{Elliptical Quanta Circumscribed by Classical Action Fix the Wigner Resolution}
...
\begin{abstract}
The Wigner function represents a quantum state in phase space. Smoothing it gives a non-negative portrait, with the smoothing scale conventionally set from outside the state. We show that the state sets the scale. The classical action Zurek used to locate sub-Planck structure is the area of the state's covariance ellipse $\pi\,\Delta x\,\Delta p$. It circumscribes a family of elliptical quanta, de Gosson's quantum blobs deforming at constant action $h/2$, one per quadrature angle. Convolving the Wigner function with the kernel of a single quantum renders it non-negative for one choice of coordinates. Integrating over the family is coordinate-free and resolves the portrait at $\pi\hbar^2/(\Delta x\,\Delta p)$, the polar dual of the classical action. The portrait is everywhere non-negative and preserves the structure of the Wigner function. Wigner resolution derives from the state's classical action alone and sharpens as the state grows.
\end{abstract}
...
\caption{\textbf{Capacity, elliptical quanta family, and resolution.} Action capacity $A$ circumscribes the elliptical quanta family $\{\beta_\theta\}$, which circumscribes resolution $a$. Across $\theta$ the blob family deforms at constant action $\beta_\theta = h/2$. At the principal angles $\theta = 0$ and $\theta = \pi/2$, the family recovers $\beta_{\theta=0}$ and $\beta_{\pi/2}$ from Fig.~\ref{fig:principal_blobs}.}
...
\label{eq:wigner}
\end{equation}
The quantum kernel $K_\theta$ is a pure-state Wigner function, the centered Gaussian whose $h/2$ ellipse is the elliptical quantum $\beta_\theta$,
\begin{equation}
